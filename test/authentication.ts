...
describe.skip("Authentication", () => {
  ...
  it("Can authenticate", async () => {
    ...
    assert.isTrue(
      ...
      `Expected isAuthenticated to be true, but returned ${client.IsAuthenticated}`
    );
  ...
  it("Can get current user", async () => {
    ...
    assert.property(
      client.CurrentUser,
      "Email",
      "Expected CurrentUser to have an Email property."
    );
    assert.equal(
      client.CurrentUser.Email,
      USERNAME,
      `Expected CurrentUser email to be ${USERNAME}, but found ${client.CurrentUser.Email}`
    );
  });
  ...
  it("Can check permissions", async () => {
    ...
    assert.property(
      client.CurrentUser,
      "hasPermission",
      "Expected CurrentUser to have a hasPermission() function."
    );
    assert.isFunction(
      client.CurrentUser.hasPermission,
      `CurrentUser.hasPermission expected to be a function, but is ${typeof client
        .CurrentUser.hasPermission}`
    );
    assert.isTrue(
      client.CurrentUser.hasPermission("ACCESS_ADMIN"),
      "Expected CurrentUser to have ACCESS_ADMIN permission."
    );
    assert.isFalse(
      client.CurrentUser.hasPermission("UNKNOWN_PERMISSION"),
      "Expected CurrentUser to NOT have UNKNOWN_PERMISSION permission."
    );
  });
  ...
  it("Can log out", async () => {
    ...
    assert.isFalse(
      client.IsAuthenticated,
      `Expected IsAuthenticated to false, but received ${client.IsAuthenticated}`
    );
